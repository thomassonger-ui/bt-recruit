...
Last updated: April 14, 2026 (guardrails locked in after ghost-credentials incident)

This file gives Claude persistent context for all BearTeamOS work sessions.

---

## GUARDRAILS — NON-NEGOTIABLE (April 2026)

Codified after the ghost-credentials incident. Breaking any of these sends BearTeamOS login emails to unauthorized recipients.

1. **`/api/onboard` is human-trigger-only.** Gated by `DASHBOARD_PASSWORD` (required, not optional). Called ONLY from the Dashboard "Mark Joined" button (`app/dashboard/page.tsx` → `confirmMarkJoined`). NEVER wire Supabase Database Webhooks, Vercel crons, Calendly webhooks, SendGrid webhooks, or any automation to this route.
...
3. **Tom `[LEAD]` alert fires exactly once per new lead.** Guarded by `!returningLead` — first email capture only.
4. **BearTeamOS credentials (Vercel `AGENTS` env var on the BearTeamOS project) are provisioned ONLY by `/api/onboard`.** No other route, webhook, cron, or script can add/modify AGENTS entries.
5. **Supabase Database Webhooks: `/api/onboard` is a forbidden destination.** Audit every webhook on `leads` before enabling. Calendly booking webhooks flow to `/api/booking-webhook` only.
6. **Agent lifecycle is one-way and human-gated:**
   `agent_prospects` → `leads(stage=scout_captured)` → `leads(status=booked)` → `leads(stage=Closed Won)` → (Tom clicks **Mark Joined**) → `leads(stage=Onboarding)` + `agents` row + AGENTS env var + welcome email.
7. **Scout runs on OpenAI gpt-4o. Claude is system architect only — not deployed in any Bear Team product.**

### Route ownership

| Route | Purpose | Allowed callers |
|---|---|---|
| `app/api/chat/route.ts` | LIVE Scout route | Public chat UI |
| `app/api/scout/route.ts` | Unified route (exists, NOT wired to page) | — |
| `app/api/booking-webhook/route.ts` | Calendly webhook → `leads.status=booked` | Calendly only |
| `app/api/onboard/route.ts` | Credentials + welcome email | **Dashboard "Mark Joined" button only** |

### Fixed April 2026

- Ghost credentials email from `/api/onboard` firing on Supabase lead updates — webhook killed + `DASHBOARD_PASSWORD` required (commit `26e6d32`).
- Duplicate `[LEAD]` alert emails (3 per lead) — `!returningLead` guard (commit `8d49b2d`).
- AGENTS env var cleaned: 4 legit entries (Tom_Songer, Beth_Baer, Veronica_Burnett, agent).
